...
if ( ! function_exists( 'wmcc_display_content_copier' ) ) {
    ...
    function wmcc_display_content_copier() {
        ...
        if ( $sites != null ) {
            ...
            if ( $wmcc_blogs != null ) {                
                ?>
                <p><strong><?php esc_html_e( 'Select destination sites you want to copy or update.', 'wp-multisite-content-copier' ); ?></strong></p>
                    ...
                    <?php
                        ...
                        foreach ( $sites as $key => $value ) {
                            if ( in_array( $value->blog_id, $wmcc_blogs ) && $value->blog_id != get_current_blog_id() ) {
                                ...
                                ?>
                                    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input type="checkbox" value="<?php echo (int) $value->blog_id; ?>"<?php echo esc_attr( $checked ); ?>><?php echo esc_html( $value->domain ); echo esc_html( $value->path ); echo ' ('.esc_html( $blog_details->blogname ).')'; ?> <strong><?php echo esc_html( $post_modified ); ?></strong></label></p>
                                <?php
                            }
                        }
                    ?>
                </div>
                <p><strong><?php esc_html_e( 'Extra Options', 'wp-multisite-content-copier' ); ?></strong></p>
                <?php
                    if ( $type == 'post_type' ) {
                        ?>
                            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input id="is-copy-media" type="checkbox" value="1"><?php esc_html_e( ' Copy or update media ( Attachments )', 'wp-multisite-content-copier' ); ?></label></p>
                        <?php
                        if ( $type_name == 'post' ) {
                            ?>
                                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input id="is-copy-terms" type="checkbox" value="1"><?php esc_html_e( ' Copy or update terms ( Categories & Tags )', 'wp-multisite-content-copier' ); ?></label></p>
                            <?php
                        }
                    }
                ?>                
                <button type="button" id="wmcc-copy" class="button-primary"><?php esc_html_e( 'Copy/Update', 'wp-multisite-content-copier' ); ?></button>            
                <?php      
            } else {
                ?>
                <div id="wmcc-notice">
                    <div class="wmcc-notice-warning">
                        <p><?php esc_html_e( 'Sites is not selected. Please go to Network Admin->WMCC and select sites.', 'wp-multisite-content-copier' ); ?></p>
                    </div>
                </div>
                <?php
            }           
